...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    {
        $order = $_POST["itemsToDelete"];
        $sql2 = "Delete from Orders where O_id=:item";
        $stmt2=$pdo->prepare($sql2);
        $stmt2->bindParam(":item", $order);
        if($stmt2->execute())
        {
            $deleteMessage ="Order $order has been deleted";
        }
        else{
            $deleteMessage = "Unable to delete order number $order";
        }
    }
}
    ...
    <div class="table-wrapper">
        ...
        <table id="table">
            ...
            <tr>
                ...
                <td><?=$row["items"]?></td>
                <td>$<?=$row["Total"]?></td>
                <td><input type="radio" name="itemsToDelete" value="<?=$row["O_id"]?>"></td>
            </tr>
            <?php
            $rownum++;
            }
